<template>
 ...
 <el-tree
  ...
  ref="tree2">
   ...
   <el-button type="primary" @click='check()'>level搜索</el-button>
                     <el-table
      :data="img"
      style="width: 100%">
      <el-table-column
  ...
  // 生命周期钩子
   ...
   async check(){
    ...
    .then(function(result){
      ...
      handleClick(row) {
          console.log(row) 
          this.$store.state.saveInfo.question_image_id = row.image_id
          this.$store.state.answerList.image_id = row.image_id
           this.$store.state.answerList.image_url = row.url
           this.$store.state.questionList.question_image_id = row.image_id
           //this.$store.state.questionList.image_url = row.url
          this.$store.state.saveInfo.question_image_url = row.url
          this.$store.state.editAnswer.image_id = row.image_id
          this.$store.state.editImg.image_id = row.image_id
          this.$store.state.editImg.url = row.url
          this.$store.state.editInfo.question_image_url = row.url
          this.$store.state.editInfo.question_image_id = row.image_id
          ...
          if(this.$store.state.editAnswer.source === 4){
           ...
           if(this.$store.state.editImg.source === 5){
            this.$router.push({name:'imageedit',params:this.$store.state.editImg}) 
